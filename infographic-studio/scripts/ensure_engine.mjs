...
// -*- coding: utf-8 -*-
/**
 * ensure_engine.mjs — Garantiza que el motor de infographic-studio esté listo SIN que el
 ...
 * El motor Node vive en el node_modules de la carpeta de la skill (render_infographic.mjs
 * es fijo). A diferencia de deck-studio, esta skill NO usa pptxgenjs ni python-pptx: no
 * hay .pptx aquí, la salida es un PNG (y un PDF de una página).
 *
...
if (nodeDepsOk()) {
  log(`Node OK: playwright presente en ${SKILL_DIR}.`);
} else {
  log(`Instalando playwright en ${SKILL_DIR}…`);
  try {
    execSync(`npm install playwright --prefix "${SKILL_DIR}"`, { stdio: "inherit", windowsHide: true });
...
const py = pyCmd();
if (!py) { warn("No encuentro Python 3. Instálalo con img2pdf para el PDF."); process.exit(2); }
function pyDepsOk() {
  try { execSync(`${py} -c "import img2pdf"`, { stdio: "ignore", windowsHide: true }); return true; } catch { return false; }
}
if (pyDepsOk()) {
  log("Python OK: img2pdf presente.");
} else {
  log("Instalando img2pdf…");
  try {
    execSync(`${py} -m pip install img2pdf`, { stdio: "inherit", windowsHide: true });
  } catch (e) { warn(`No pude instalar img2pdf (${e.message}). Manual: ${py} -m pip install img2pdf`); process.exit(2); }
}
log("Motor de infographic-studio listo.");
process.exit(0);
